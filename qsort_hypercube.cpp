...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	// MPI_Allreduce computes the sum of local_median values across all processes
	// in sub_hypercube_comm; the result (sum of medians) is stored in pivot.
	// Dividing by sub_hypercube_size gives the mean (computed after this call).
	...
	if (nbr_k > my_id) {
	    // This process has a lower rank: it keeps elements <= pivot
	    // and exchanges the ">" portion with its higher-ranked neighbor.

	    // MPI-2: Send number of elements greater than pivot to neighbor
	    MPI_Send(&list_size_gt, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);

	    // MPI-3: Receive number of elements <= pivot that neighbor will send
	    ...
	    // MPI-4: Send list[idx ... list_size-1] (elements > pivot) to neighbor
	    MPI_Send(&list[idx], list_size_gt, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);

	    // MPI-5: Receive neighbor's list of elements that are <= pivot
	    ...
	    // This process has a higher rank: it keeps elements > pivot
	    // and exchanges the "<=" portion with its lower-ranked neighbor.

	    // MPI-6: Receive number of elements > pivot that lower-ranked neighbor will send
	    MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD, &status);

	    // MPI-7: Send number of elements <= pivot to lower-ranked neighbor
	    MPI_Send(&list_size_leq, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);

	    // Allocate storage for neighbor's list
	    nbr_list = new int[nbr_list_size]; 

	    // MPI-8: Receive neighbor's list of elements that are > pivot
	    MPI_Recv(nbr_list, nbr_list_size, MPI_INT, nbr_k, tag, MPI_COMM_WORLD, &status);
